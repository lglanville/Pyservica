...
def get_iname(filename, ident):
    """Takes a filname and identifier and returns an appropriate name for the
    intellectual object"""
    if filename.suffix in ('.wav', '.oma', '.mp3'):
...
def dir_scan(dir, identifier, sip, base, security='open', write=True):
    ...
    for root, _, files in os.walk(dir):
        for file in files:
            ...
            id = id_transform(fpath.name)
            if id == identifier:
                checksum = siplib.sha256sum(fpath)
                if not find_dupe(checksum, sip):
                    iname = get_iname(fpath, identifier)
                    if iname not in sip.get_infobjs().values():
                        info_ref = sip.add_infobj(iname, base, security_tag=security)
                    else:
                        info_ref = [key for key, val in sip.get_infobjs().items() if val == iname][0]
                    c_object = sip.add_contobj(relpath.name, info_ref, security_tag=security)
                    label, type = get_rep(fpath.as_posix())
                    sip.add_representation(label, info_ref, [c_object], type=type)
                    if label == 'Preservation original':
                        sip.add_generation(c_object, '', [relpath], orig='false', active='false')
                    elif label == 'Access':
                        sip.add_generation(c_object, '', [relpath], orig='false', active='true')
                    else:
                        sip.add_generation(c_object, '', [relpath])
                    checksum = siplib.hash_file(fpath, ['SHA256', 'SHA512'])
                    sip.add_bitstream(relpath, checksum, write=write)
...
def audio(metadata, dirs, outdir, parent, security='open', write=True):
